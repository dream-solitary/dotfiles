...
myKeys home = [
         ...
         , ("M-c", namedScratchpadAction myScratchPads "mocast")
    ]
...
myScratchPads = [ NS "pulsemixer" spawnPulsemixer findPulsemixer managePulsemixer
                , NS "quodlibet" "quodlibet" (className =? "Quodlibet") ((customFloating $ W.RationalRect 0.1 0.1 0.8 0.8) <+> doF W.swapUp)
...
myManageHook = (isDialog --> doF W.swapUp)
     ...
     , className =? "Kodi"    --> doFullFloat <+> doShift (myWorkspaces !! 3 )
     --, className =? "vlc"     --> doFloat <+> doShift ( myWorkspaces !! 3 )
...
myStartupHook = do
          ...
          spawnOnce "fcitx"  -- fcitx framework
